import { X_AUTH_ENDPOINTS, type XAuthConfig } from "./config";
...
export async function generateAuthUrl(config: XAuthConfig): Promise<AuthUrlResult> {
  ...
  const url = `${X_AUTH_ENDPOINTS.authorize}?${params.toString()}`;
...
export async function exchangeCodeForToken(params: ExchangeCodeParams): Promise<TokenResponse> {
  ...
  const response = await fetch(X_AUTH_ENDPOINTS.token, {
    method: "POST",
    headers,
    body: body.toString(),
  });
...
export async function revokeToken(
  ...
  const response = await fetch(X_AUTH_ENDPOINTS.revoke, {
    method: "POST",
    headers,
    body: body.toString(),
  });
