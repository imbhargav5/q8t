import { DISCORD_AUTH_ENDPOINTS, type DiscordAuthConfig } from "./config";
...
export async function generateAuthUrl(config: DiscordAuthConfig): Promise<AuthUrlResult> {
  ...
  const url = `${DISCORD_AUTH_ENDPOINTS.authorize}?${params.toString()}`;
...
export async function exchangeCodeForToken(params: ExchangeCodeParams): Promise<TokenResponse> {
  ...
  const response = await fetch(DISCORD_AUTH_ENDPOINTS.token, {
    method: "POST",
    headers,
    body: body.toString(),
  });
...
export async function revokeToken(
  ...
  const response = await fetch(DISCORD_AUTH_ENDPOINTS.revoke, {
    method: "POST",
    headers,
    body: body.toString(),
  });
