...
function extractEndpointInfo(path: string, method: string, operation: Operation): EndpointInfo {
  ...
  let requestBodyOptional = true;
  if (operation.requestBody) {
    const jsonContent = operation.requestBody.content["application/json"];
    if (jsonContent) {
      const schema = jsonContent.schema;
      requestBodyType = getTypeName(schema);
      requestBodyOptional = !operation.requestBody.required;
    }
  }
...
function getTypeName(schema: SchemaObject | RefObject): string {
  ...
  // Handle allOf, oneOf
  if (schema.allOf) {
    const types = schema.allOf.map((s) => getTypeName(s));
    return types.join(" & ");
  }

  if (schema.oneOf) {
    const types = schema.oneOf.map((s) => getTypeName(s));
    return `(${types.join(" | ")})`;
  }
...
function generateMethodParams(endpoint: EndpointInfo): string {
  ...
  // Request body
  if (endpoint.requestBodyType) {
    const optional = endpoint.requestBodyOptional ? "?" : "";
    params.push(`body${optional}: ${endpoint.requestBodyType}`);
  }
