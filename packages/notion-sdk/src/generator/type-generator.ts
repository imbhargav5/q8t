...
function generateInterface(name: string, schema: SchemaObject): string {
  ...
  if (schema.allOf) {
    ...
    for (const subSchema of schema.allOf) {
      if (isRefObject(subSchema)) {
        types.push(resolveRef(subSchema.$ref));
      } else if (subSchema.properties) {
        // Inline the properties
        types.push(`{ ${generatePropertiesInline(subSchema)} }`);
      }
    }
...
function generatePropertiesInline(schema: SchemaObject): string {
  const props: string[] = [];
  if (schema.properties) {
    for (const [propName, propSchema] of Object.entries(schema.properties)) {
      const isRequired = schema.required?.includes(propName) ?? false;
