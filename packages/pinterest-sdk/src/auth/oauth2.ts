import { PINTEREST_AUTH_ENDPOINTS, type PinterestAuthConfig } from "./config";
...
export async function generateAuthUrl(config: PinterestAuthConfig): Promise<AuthUrlResult> {
  ...
  const url = `${PINTEREST_AUTH_ENDPOINTS.authorize}?${params.toString()}`;
...
export async function exchangeCodeForToken(params: ExchangeCodeParams): Promise<TokenResponse> {
  ...
  const response = await fetch(PINTEREST_AUTH_ENDPOINTS.token, {
    method: "POST",
    headers,
    body: body.toString(),
  });
