...
export function generateApi(spec: OpenAPISpec, apiName: string): string {
...
function extractEndpointInfo(path: string, method: string, operation: Operation): EndpointInfo {
  ...
  if (operation.requestBody?.content["application/json"]) {
    ...
    requestBodyType = getTypeName(schema);
  } else if (operation.requestBody?.content["text/csv"]) {
    requestBodyType = "string";
  }
  ...
  } else if (successResponse?.content?.["text/csv"]) {
    responseType = "string";
  }
